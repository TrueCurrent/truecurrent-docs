...
description: "Comprehensive FAQ covering TrueCurrent's RFQ trading model, non-custodial security, zero fees, liquidity provider program, liquidation mechanics, funding rates, and wallet authorization on Injective."
...
<AccordionGroup>
  <Accordion title="What is TrueCurrent?">
    ...
    Instead of trading against an automated liquidity provider or a passive order book, you receive live, competitive quotes from professional liquidity providers and execute at the locked-in price with no slippage.
  </Accordion>
  ...
  <Accordion title="What fees does TrueCurrent charge?">
    ...
    Liquidity providers pay a 4 bps (0.04%) protocol fee on filled volume, which is how the protocol sustains itself while keeping trading free for users. See [Fees](/trading/fees) for the full breakdown.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="What is an RFQ?">
    RFQ (Request for Quote) is TrueCurrent's trading model where professional liquidity providers compete in real time to fill your order at a fixed, firm price.
  ...
  <Accordion title="What happens if no liquidity provider responds to my request?">
    Your order automatically routes to Injective's onchain central limit order book if no liquidity provider responds within the 2-second window.
  ...
  <Accordion title="Why did my quote expire before I could accept?">
    Quotes expire after a short window (typically 30 seconds) because liquidity providers need to manage price risk in fast-moving markets.
...
<AccordionGroup>
  <Accordion title="How do I become a liquidity provider?">
    Apply for whitelist access by contacting the TrueCurrent team and completing a testnet integration.

    You'll need to demonstrate your system is working correctly on testnet before being approved. See [Getting whitelisted](/liquidity-providers/getting-whitelisted) for the full process.
  </Accordion>
  ...
  <Accordion title="Why is my quote not winning trades?">
    Your price is likely less competitive than other liquidity providers.
  ...
  <Accordion title="Can I see who other liquidity providers are?">
    Liquidity provider addresses are visible onchain via `AcceptQuote` transaction history, but the whitelist is not publicly published.

    You can observe trade history to infer which makers are currently active.
  </Accordion>

  <Accordion title="What fees do liquidity providers pay?">
    Liquidity providers pay a 4 bps (0.04%) protocol fee on filled volume; traders pay nothing.

    Revenue for liquidity providers comes from the spread between bid and ask, net of the 4 bps fee. See [Fees](/trading/fees) and [Liquidity provider overview](/liquidity-providers/overview) for details.
  </Accordion>

  <Accordion title="How is liquidity provider performance monitored?">
    TrueCurrent monitors liquidity providers on response rate, price competitiveness, settlement success rate, and overall quoting behavior.

    Consistently failing to respond within the 2-second window, quoting off-market prices, or allowing settlements to fail due to insufficient margin will negatively affect your standing and may result in removal from the whitelist. See [Getting whitelisted](/liquidity-providers/getting-whitelisted) for the full criteria.
  </Accordion>
</AccordionGroup>
