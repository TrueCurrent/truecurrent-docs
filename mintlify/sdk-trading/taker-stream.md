...
The connection is **gRPC-web over WebSocket**. The URL is `<host>/<service>/<method>` where the service is `injective_rfq_rpc.InjectiveRfqRPC` and the method for takers is `TakerStream` (market makers use `MakerStream`). Messages are framed with gRPC-web length prefixes and use protobuf payloads. The `rfq-testing` Python library and reference examples handle this framing for you. If you're building from scratch, see `src/rfq_test/clients/websocket.py` for the implementation.
...
| `nonce` | uint64 \| null | Present on "blind" quotes (see [Best practices](/sdk-trading/taker-best-practices)) |
